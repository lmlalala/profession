...
const LANG_CONFIG = {
  ...
  js:         { label: 'JS',   color: 'rgba(247,223,30,0.15)',  border: 'rgba(247,223,30,0.4)',  text: '#f7df1e' },
}
...
function runConsole(code, outputEl) {
  ...
  try {
    ...
    if (result && typeof result.then === 'function') {
      result
        .catch(e => writeToConsole(outputEl, [`❌ ${e.message}`], 'error'))
        .finally(restore)
    } else {
      restore()
    }
...
async function initMultiTabCodeBlock(codeBlockEl) {
  ...
  const tabsHtml = tabLangs.map((lang, i) => {
